...
- **测试目的**：验证单文件回放控制接口的幂等性。
- **流程与逻辑**：未回放时依次调用 `pauseBinReplay`/`resumeBinReplay`/`stopBinReplay`；回放中重复调用 `pauseBinReplay`（两次）、重复调用 `resumeBinReplay`（两次）；`stopBinReplay` 停止后再次调用 `stopBinReplay`。
- **预期结果**：各次调用返回 OK 或被明确拒绝（状态不符时），不崩溃、不抛异常。
- **有效性说明**：单文件回放控制接口在重复调用/状态不符时的健壮性。
- **可自动化**：auto
- **人工介入**：无
- **测试结果**：待测试
