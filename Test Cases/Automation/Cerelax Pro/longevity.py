...
用法：
  ...
  python longevity.py 850B       # 指定目标 identity（须在 config.DEVICES 中定义）
...
def _dump_sample_rates(sensor):
    ...
    覆盖 DeviceInfo 的采样率字段 + EEG_SAMPLE_RATE 参数（Cerelax Pro 为 EEG 头环，支持该参数）。
    """
    try:
...
def main():
    ...
    print("  - 待测设备：Cerelax Pro 头环 上电、在范围内", flush=True)

    input("\n>>> [人工操作] 请确认：1) 系统蓝牙已【关闭】；2) USB dongle 已插入；"
          "3) Cerelax Pro 头环 已【开机】且在范围内。本脚本将长时间运行，完成后按回车开始 ...")
