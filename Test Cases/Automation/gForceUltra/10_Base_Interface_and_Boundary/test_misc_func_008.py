...
流程：
  1) 确认设备开机 -> 按回车
  2) scan 匹配 gForceUltra -> requireSensor -> connect -> init
  3) getDeviceInfo() 获取 DeviceInfo 对象
  4) 逐一读取 ModelName 及各模态 ChannelCount/SampleRate
  5) 断言字段存在且值 ≥0；gForceUltra 的 EMG/IMU 类字段 >0，EEG/ECG 类为 0
"""
...
def main():
    ...
    # 检查3：gForceUltra 预期 >0 的字段
    positive_ok = True
    for f in GFORCE_EXPECTED_POSITIVE:
        v = field_values.get(f)
        if isinstance(v, (int, float)):
            if v <= 0:
                print(f"[检查3] {f} = {v}（预期 >0）", flush=True)
                positive_ok = False
    record(results, "gForceUltra 预期字段（EMG/IMU/ACC/GYRO 等）>0", positive_ok,
           "gForceUltra 的 EMG/IMU/Acc/Gyro 等字段 >0",
           "全部 >0" if positive_ok else "存在 ≤0")

    # 检查4：gForceUltra 预期 =0 的字段（EEG/ECG/PPG/SPO2/BRTH）
    zero_ok = True
    for f in GFORCE_EXPECTED_ZERO:
        v = field_values.get(f)
        if isinstance(v, (int, float)):
            if v != 0 and v is not None:
                print(f"[检查4] {f} = {v}（预期 =0 或 None）", flush=True)
                zero_ok = False
    record(results, "gForceUltra 非预期字段（EEG/ECG/PPG/SPO2/BRTH）=0", zero_ok,
           "gForceUltra 的 EEG/ECG/PPG/SPO2/BRTH 等字段 =0",
