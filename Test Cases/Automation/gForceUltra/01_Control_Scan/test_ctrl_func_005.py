...
def main():
    ...
    # 检查4：回调列表含 gForceUltra 设备
    has_gforce_ultra = any(any(nm.upper().startswith('GFORCEULTRA') for nm in names) for _, names in found_events)
    print(f"[检查4] 回调列表含 gForceUltra 设备 = {has_gforce_ultra}", flush=True)
    record(results, "onDeviceFoundCallback 列表含 gForceUltra 设备", has_gforce_ultra,
           "回调列表含 Name 以 gForceUltra 开头的设备", f"含 gForceUltra={has_gforce_ultra}")
    ...
    if target is None:
        print("[检查5] 未发现 gForceUltra 设备，跳过“已连接设备不重复推送”检查", flush=True)
        record(results, "已连接设备不重复推送", False,
               "连接 gForceUltra 后回调不再含该设备", "未发现 gForceUltra 设备，未执行连接")
    else:
        ...
        if sensor is None:
            ...
            record(results, "已连接设备不重复推送", False,
                   "连接 gForceUltra 后回调不再含该设备", "requireSensor 返回 None")
        else:
            try:
                ok = sensor.connect()
            except Exception as e:
            ...
            if not ok:
                ...
                record(results, "已连接设备不重复推送", False,
                       "连接 gForceUltra 后回调不再含该设备", f"connect 返回 {ok}")
            else:
                time.sleep(1)
                found_events.clear()
                t0 = time.time()
                ...
                record(results, "SensorController.stopScan 不抛异常（检查5内）", not stop_raised,
                       "SensorController.stopScan 不抛异常", f"抛异常 {stop_err}" if stop_raised else "无异常")
                still_contains = any(any(nm.upper().startswith('GFORCEULTRA') for nm in names) for _, names in found_events)
                print(f"[检查5] 连接后回调仍含 gForceUltra = {still_contains}（预期 False）", flush=True)
                record(results, "已连接设备不重复推送", not still_contains,
                       "连接 gForceUltra 后回调不再含该设备", f"连接后回调含 gForceUltra={still_contains}")
                try:
                    sensor.disconnect()
                except Exception as e:
