...
流程：
  ...
  3) scan 发现 gForceUltra -> requireSensor -> connect
...
def main():
    ...
    if target is None:
        print("[检查] 未发现 gForceUltra 设备", flush=True)
        record(results, "连接后 getConnectedSensors 含该 mac", False,
               "getConnectedSensors 含目标 mac", "未发现 gForceUltra 设备")
        record(results, "连接后 getConnectedDevices 含该 mac", False,
               "getConnectedDevices 含目标 mac", "未发现 gForceUltra 设备")
    else:
