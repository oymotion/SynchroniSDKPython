...
def main():
    ...
    if target is None:
        print("[检查4] 未匹配到 config 中启用的设备（gForceUltra/80F3）", flush=True)
        record(results, "SensorController.scan 发现目标设备 gForceUltra", False,
               "scan 返回含 gForceUltra", f"发现 {n} 台，未匹配到目标")
    else:
        name = getattr(target, 'Name', '?')
        addr = getattr(target, 'Address', '?')
        print(f"[检查4] 匹配到目标设备: {name} {addr}", flush=True)
        record(results, "SensorController.scan 发现目标设备 gForceUltra", True,
               "scan 返回含 gForceUltra", f"匹配到 {name} {addr}")
