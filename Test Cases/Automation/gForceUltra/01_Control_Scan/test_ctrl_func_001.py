...
流程（注意：这里的"蓝牙"指【电脑】蓝牙，不是待测设备 gForceUltra）：
  1) 人工关闭电脑蓝牙 -> 按回车
     （此时待测设备 gForceUltra 保持开机，用于证明"设备在范围内但电脑蓝牙关闭仍扫不到"）
  2) 断言 isEnable == False
  3) 断言 startScan 被拒绝（未进入扫描态 isScanning==False）
  4) 断言 scan 无结果（电脑蓝牙关闭不应扫到设备）
  5) 人工重新开启电脑蓝牙 -> 按回车
     （此阶段只检查电脑蓝牙 isEnable==True，不扫描、不连接设备，
       故 gForceUltra 保持开机或关机均可，不影响结果）
...
def main():
    ...
    # ---- 阶段 1：关闭蓝牙 ----
    input("\n>>> [人工操作] 请【关闭电脑】蓝牙（不是待测设备 gForceUltra），并保持 gForceUltra 设备开机，完成后按回车继续 ...")
    ...
    # ---- 阶段 2：重新开启蓝牙 ----
    input("\n>>> [人工操作] 请【开启电脑】蓝牙（不是待测设备 gForceUltra），完成后按回车继续 ..."
          "\n    （本阶段只检查电脑蓝牙 isEnable，不扫描、不连接设备；设备 gForceUltra 保持开机或关机均可，不影响结果）")
