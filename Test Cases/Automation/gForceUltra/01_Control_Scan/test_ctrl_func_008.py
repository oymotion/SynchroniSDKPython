...
流程：
  ...
  3) scan 发现 gForceUltra -> requireSensor 创建 profile
...
def main():
    ...
    if target is None:
        print("[检查1] 未发现 gForceUltra 设备", flush=True)
        record(results, "requireSensor(有效设备) 返回 SensorProfile", False,
               "requireSensor(有效 BLEDevice) 返回 SensorProfile", "未发现 gForceUltra 设备")
        print("[检查2] 未发现 gForceUltra 设备，无法验证命中", flush=True)
        record(results, "getSensor(正确 mac) 返回 profile", False,
               "getSensor(正确 mac) 返回 SensorProfile", "未发现 gForceUltra 设备")
    else:
