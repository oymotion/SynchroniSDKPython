...
前置条件：
  - 主机(电脑)：蓝牙已开启
  - 待测设备：gForceUltra 需先开机（用于扫描获取 BLEDevice），再关闭电源制造连接失败

流程：
  1) 确认设备开机 -> 按回车
  2) scan 匹配 gForceUltra -> 记录 BLEDevice
...
def main():
    ...
    print("  - 待测设备：gForceUltra 需先开机（扫描获取 BLEDevice），再关闭电源制造连接失败", flush=True)
    ...
    if target is None:
        print("[FAIL] 未匹配到 config 中启用的设备（gForceUltra/80F3）", flush=True)
        record(results, "scan 匹配到目标设备", False, "scan 返回含 gForceUltra", "未匹配到目标")
        print("\n结论: FAIL", flush=True)
        ctrl.terminate()
        return
    ...
    record(results, "scan 匹配到目标设备", True, "scan 返回含 gForceUltra", f"匹配到 {name} {addr}")

    # ---- 阶段 2：人工关闭设备，制造连接失败 ----
    input("\n>>> [人工操作] 请【关闭】待测设备 gForceUltra 电源（或将其移出蓝牙范围），完成后按回车继续 ...")
