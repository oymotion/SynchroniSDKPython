# -*- coding: utf-8 -*-
"""OB6000A 脑电传感器设备规格（Spec）——单一事实来源。

本文件是该型号「应该支持」的能力基线，测试用例据此硬断言：
设备行为偏离 spec 即 FAIL（而非 SKIP / 自适应跳过）。

同一型号（name_prefix == "OB6000A"）的多台设备共用本文件，
通过 config.MODEL_SPEC 映射、common.load_spec() 读取，无需逐次确认。

数据来源（实测固化，2026-09-14，设备 206F，probe_device_info.py dump）：
  - DeviceName = 'OB6000A(206F)'，ModelName = 'OB6000'，Hardware 0.0
  - Firmware V1.0.31_2026-02-11_ebc0e72
  - EegChannelCount = 16；ImpeChannelCount = 16（16 电极接触检测）
  ...
  - EEG_SAMPLE_RATE_LIST = 250|500（EegMaxSampleRate=500，上电默认 250）
  - IMU_SAMPLE_RATE_LIST = 50（固定 50Hz）
  - 采样率 key 均可 getParam/setParam（见 examples/SynchroniSDKPython_DemoNewMulti.py）

与 OB6000C 的差异（重要，A/C 靠广播名型号区分，ModelName 均为 'OB6000'）：
  - EEG 通道：16（OB6000C 为 32）
  - Impedance 通道：16（OB6000C 为 32）
  - EEG 采样率：250|500 两档（OB6000C 固定 250）
...
命名说明（勿混用）：
  - 本文件 streams 的 key 采用 setParam/getParam 开关 key（NTF_GFORCE_ACC 等），
    与 gforce_ultra / cerelax_pro / ob6000c 的 spec 一致。
...
SPEC = {
    # ===== 身份 =====
    "model": "OB6000A",
    "type": "脑电传感器（16 通道 EEG + 6 轴 IMU + 16 通道电极接触检测）",
    "name_prefix": "OB6000A",
    ...
    "streams": {
        ...
        "NTF_IMPEDANCE":    {"supported": True,  "channels": 16},  # ImpeChannelCount=16（电极接触检测）
        "NTF_EEG":          {"supported": True,  "channels": 16},  # EegChannelCount=16（核心）
    ...
    "sample_rates": {
        "EEG_SAMPLE_RATE": {"rates": ["250", "500"], "default": "250"},
