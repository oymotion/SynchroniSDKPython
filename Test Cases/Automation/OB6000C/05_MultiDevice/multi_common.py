...
def connect_and_init_all(ctrl, matched, results):
    ...
    for tid, device in matched:
        ...
        hinited = sensor.hasInited
        state = sensor.deviceState
        print(f"[init 后] {tid}: hasInited={hinited}, deviceState={state}", flush=True)
        record(results, f"init 后 hasInited==True({tid})", hinited is True,
               "init 返回 True 后 hasInited==True", f"hasInited={hinited}")
        record(results, f"init 后 deviceState==Ready({tid})", state == DeviceStateEx.Ready,
               "init 返回 True 后 deviceState==Ready", f"deviceState={state}")
