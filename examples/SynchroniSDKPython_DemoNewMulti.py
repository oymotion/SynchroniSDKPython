    ...
    def _connect_selected_device(self):
        ...
        if not sensor.hasInited:
            ...
            if state.bio_kind == "emg":
                status_parts.append(f"EMG {info.EmgChannelCount}ch @ {info.EmgSampleRate}Hz")
            else:
                status_parts.append(f"EEG {info.EegChannelCount}ch @ {info.EegSampleRate}Hz")
                status_parts.append(f"ECG {info.EcgChannelCount}ch @ {info.EcgSampleRate}Hz")
                status_parts.append(f"BRTH {info.BrthChannelCount}ch @ {info.BrthSampleRate}Hz")
            state.status_text = " | ".join(status_parts)
